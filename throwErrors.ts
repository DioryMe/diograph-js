...
export function throwErrorIfLinkNotFound(method: string, linkObject: ILinkObject, links?: ILinkObject[]): void {
...
export function throwErrorIfLinkAlreadyExists(method: string, linkObject: ILinkObject, links?: ILinkObject[]): void {
  if (!links?.find(({ id }) => id === linkObject.id)) {
    return
  }
